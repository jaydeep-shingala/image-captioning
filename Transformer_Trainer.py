...
# df = pd.read_csv("imgfeature_captions.csv", names=['image_features','captions'])
# image_features = df['image_features']
# image_features = [get_tensor(tensor_str) for tensor_str in image_features]
...
print("-------------------------------------------")
torch.save(transformer.state_dict(), "image_caption_transformer_model_2.pt")
print("model saved")
transformer.eval()
def translate(eng_sentence1):
  eng_sentence = (eng_sentence1,)
  kn_sentence = ("",)
  for word_counter in range(max_sequence_length):
    encoder_self_attention_mask, decoder_self_attention_mask, decoder_cross_attention_mask= create_masks(eng_sentence, kn_sentence)
    predictions = transformer(eng_sentence1,
                              kn_sentence,
                              encoder_self_attention_mask.to(device), 
                              decoder_self_attention_mask.to(device), 
                              decoder_cross_attention_mask.to(device),
...
# print(image_features[0].shape)
# print(translate(image_features[0].unsqueeze(0)))

with open("test_Predicted_text_captions.txt", 'w') as f, open("test_org_text_captions.txt", 'w') as f2:
    for i in range(50):
        predicted_cap = translate(image_features[i].unsqueeze(0))
        f.write(predicted_cap+'\n')
        f2.write(captions[i]+'\n')
...
plt.savefig("epoch_trainingloss-2.png")